    ...
    public class adBaiHatsController : Controller
    {
      ...
      /*  public ActionResult Index(string sortOrder)
        ...
        public ActionResult Create([Bind(Include = "Id,TenBH,CaSi,TacGia,NgayCapNhat,HinhAnhBH,UrlBH,LoiBH,MaTL")] BaiHat baiHat, HttpPostedFileBase img, HttpPostedFileBase url)
        {
            ...
            {
                //// Đảm bảo mỗi bài hát có thời gian khác nhau
                //lastUpdateTime = lastUpdateTime.AddSeconds(1);
                //baiHat.NgayCapNhat = lastUpdateTime;

                //// Cập nhật thời gian hiện tại
                //baiHat.NgayCapNhat = DateTime.Now;


                Random rnd = new Random();
                int secondsOffset = rnd.Next(1, 60);

                // Cập nhật thời gian hiện tại + số giây ngẫu nhiên
                baiHat.NgayCapNhat = DateTime.Now.AddSeconds(secondsOffset);
        ...
        public ActionResult Edit([Bind(Include = "Id,TenBH,CaSi,TacGia,NgayCapNhat,HinhAnhBH,UrlBH,LoiBH,MaTL")] BaiHat baiHat)
        {
            if (ModelState.IsValid)
            {
                ...
                BaiHat existingBaiHat = db.BaiHats.Find(baiHat.Id);
                if (existingBaiHat == null)
                {
                    return HttpNotFound();
                }

                // Cập nhật thông tin từ người dùng nhập vào
                db.Entry(existingBaiHat).CurrentValues.SetValues(baiHat);

                // Cập nhật thời gian chỉnh sửa
                existingBaiHat.NgayCapNhat = DateTime.Now;
